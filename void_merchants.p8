...
function add_enemy(lvl)
	...
	enemy[5] = 199 + lvl
	-- damage
	enemy[6] = flr(10 * lvl * 0.1) + 1
	-- life
	enemy[7] = flr(20 * lvl * 0.1) + 1
	-- shields
	...
	enemy[9] = flr(lvl / 5) + 1
	-- shot_spee
	enemy[10] = flr(lvl / 5) + 1
	-- speed
	enemy[11] = flr(lvl / 5) + 1
	-- value
	enemy[12] = lvl
	--wobble
...
function get_enemy_htbx_skp_pxl_width(lvl)
 ...
 if tier == 2 or 8 or 9 or 10 or 12 or 14 or 20 then
 	return {0, 8}
 end
 if tier == 4 or 5 or 6 then
 	return {2, 5}
 end
 if tier == 7 or 13 or 15 or 16 or 17 or 19 then
 	return {1, 7}
 end
 if tier == 18 then
 	return {1, 6}
